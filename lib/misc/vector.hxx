...
namespace misc
{
    ...
    template <class T>
    bool vector<T>::has(const T& data) const
    {
        return ranges::find(*this, data) != this->end();
    }
    ...
    template <class T>
    inline vector<T>& vector<T>::remove(const T& data)
    {
        precondition(has(data));
        this->erase(ranges::find(*this, data));
        return *this;
    }

    template <class T>
    std::ostream& vector<T>::dump(std::ostream& ostr) const
    {
    ...
    auto position_append_and_move(std::vector<T>& v1, Iterator v1_iterator,
                                  std::vector<T>&& v2)
    {
