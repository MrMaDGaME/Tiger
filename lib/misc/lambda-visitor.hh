...
namespace misc
{
  /** \brief Allows to build a visitor from lambdas to use with std::visit.
     ...
     ** static type.
     **
     ** The really powerful thing here is that this can be used to match
     *multiple
     ...
     **
     **
     ** Be careful , you must necessarily have a matching lambda for any
     *possible
  ...
  template <class... Ts> struct LambdaVisitor : Ts...
  {
    /** C++ actually desugars lambdas to some kind of struct functors.
         ...
         ** } f;
         **
         ** We therefore want to access the operator() of every lambda/functor
         *used
