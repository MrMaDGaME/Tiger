...
namespace misc
{
    ...
    template <typename M>
    concept Map = requires(M map, const typename M::key_type key,
    ...
    concept ConstIterableType = ConstIterable<Container> && std::convertible_to<
        typename Container::value_type, T>;

    // Concept describing an Iterable but adds a constraint on the value_type
    // the held type should be convertible into T
    template <typename Container, typename T>
    concept IterableType = Iterable<Container> && std::convertible_to<
        typename Container::value_type, T>;

    // Concept describing an iterator on a type T
    template <typename Iter, typename T>
    concept Iterator =
        std::forward_iterator<Iter> && std::same_as<typename Iter::value_type,
                                                    T>;
