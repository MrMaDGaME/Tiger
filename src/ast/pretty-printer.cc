...
namespace ast
{
    ...
    {
        ostr_ << "var " << e.name_get() << " := " << e.var_get() << misc::iendl;
    }
    ...
    {
        for (size_t i = 0; i < e.exps_get().size() - 1; ++i)
        {
            ostr_ << *e.exps_get().at(i) << ";" << misc::iendl;
        }

        ostr_ << *e.exps_get().at(e.exps_get().size() - 1) << misc::iendl;
    }
