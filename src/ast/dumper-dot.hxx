...
namespace ast
{
    ...
    template <typename T>
    inline void DumperDot::dump_def(const T& e) const
    {
        const ast::Ast* d = nullptr;
        if (!d)
            return;
        ostr_ << parent_id << ":def:s -> "
    ...
    inline void DumperDot::dump_chunk(const E& e, const std::string& name)
    {
        ...
        ostr_ << parent_id << " [label=<" << misc::incendl
              << "<table cellborder='0' cellspacing='0'>" << misc::incendl
              << "<tr>" << misc::incendl;
        inner_fields = 0;
    ...
    namespace
    {
        ...
        inline void node_html_tr(std::ostream& ostr, const std::string& port,
        ...
        template <typename T>
        std::string html_escape(const T& input)
        {
            std::ostringstream i;
            i << input;
    ...
    inline void
    DumperDot::node_html_ports(const std::vector<std::string>& ports)
    {
        if (inner_fields)
            node_html_separator(ostr_);
        inner_fields = 0;
